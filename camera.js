  ...
  try {
    ...
    hands.onResults(r => {
      ...
      if (r.multiHandLandmarks && r.multiHandLandmarks.length > 0) {
        ...
        // Start audio on first hand detection
        if (!audioStarted) startAudio(window.AUDIO_MODE);

        // Swipe has priority over posture gestures
        if (!swipeActive) {
        ...
        // Audio reacts to hand proximity
        if (typeof anchors !== 'undefined' && anchors.length > 0) {
          const minDist = Math.min(...anchors.map(([ax, ay]) =>
            Math.sqrt((mx-ax)**2 + (my-ay)**2)
          ));
          audioOnHandMove(minDist);
        }
...
function initKeyboard() {
  document.addEventListener('keydown', e => {
    if (e.key === '+' || e.key === '=') {
      expandFactor = Math.min(1.9, expandFactor + 0.12);
      status('EXPANDIENDO'); onExpand(); audioOnExpand();
    }
    if (e.key === '-') {
      expandFactor = Math.max(0.3, expandFactor - 0.12);
      status('CONTRAYENDO'); onContract(); audioOnContract();
    }
    if (e.key === 'r' || e.key === 'R') {
      expandFactor = 1.0; densityFactor = 1.0;
      status('RESET'); onReset(); audioOnReset();
    }
    if (e.key === 'm' || e.key === 'M') {
      startAudio(window.AUDIO_MODE);
    }
  });
}
